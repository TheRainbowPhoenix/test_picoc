...
#include "../platform/platform_classpad.h"
#include <stdio.h>
...
{
    FPUTS_DEBUG("-- BasicIOInit\n");
    BasicIOInit();
    FPUTS_DEBUG("-- HeapInit\n");
    HeapInit(StackSize);
    FPUTS_DEBUG("-- TableInit\n");
    TableInit();
    FPUTS_DEBUG("-- VariableInit\n");
    VariableInit();
    FPUTS_DEBUG("-- LexInit\n");
    LexInit();
    FPUTS_DEBUG("-- TypeInit\n");
    TypeInit();
#ifndef NO_HASH_INCLUDE
    FPUTS_DEBUG("-- IncludeInit\n");
    IncludeInit();
#endif
    FPUTS_DEBUG("-- LibraryInit\n");
    LibraryInit();
#ifdef BUILTIN_MINI_STDLIB
    FPUTS_DEBUG("-- LibraryAdd\n");
    LibraryAdd(&GlobalTable, "c library", &CLibrary[0]);
    FPUTS_DEBUG("-- CLibraryInit\n");
    CLibraryInit();
#endif
    FPUTS_DEBUG("-- PlatformLibraryInit\n");
//    PlatformLibraryInit();
}
...
#ifndef NO_HASH_INCLUDE
    IncludeCleanup();
    FPUTS_DEBUG("-- IncludeCleanup\n");
#endif
    ParseCleanup();
    FPUTS_DEBUG("-- ParseCleanup\n");
    LexCleanup();
    FPUTS_DEBUG("-- LexCleanup\n");
    VariableCleanup();
    FPUTS_DEBUG("-- VariableCleanup\n");
    TypeCleanup();
    FPUTS_DEBUG("-- TypeCleanup\n");
    TableStrFree();
    FPUTS_DEBUG("-- TableStrFree\n");
    HeapCleanup();
    FPUTS_DEBUG("-- HeapCleanup\n");
}
